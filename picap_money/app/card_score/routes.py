...
from app.models.card_score import InGame
...
def index():
    query_players = InGame.query.all()
        ...
        if no_players is not None:
            ...
            for p in range(no_players):
                ...
                if name in existed_name:
                    ...
                    new_rows = InGame(name=name, score=score, round=0)
    ...
    if request.method == 'POST':     
        ...
        if check_score != 0:
            ...
            for i in range(len(players)):
                ...
                print(i, score)
                player = InGame.query.filter_by(name=players[i][0]).first()
                print(player)
